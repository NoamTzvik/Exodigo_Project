document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- HELPER: SYNC TO TODAY ---
    function syncToToday() {
        const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        const todayName = days[new Date().getDay()];
        let targetDay = '1'; // Default to Monday
        
        if (todayName === 'Monday') targetDay = '1';
        else if (todayName === 'Tuesday') targetDay = '2';
        else if (todayName === 'Wednesday') targetDay = '3';
        else if (todayName === 'Thursday') targetDay = '4';
        else if (todayName === 'Friday') targetTargetDay = '5';
        else targetDay = 'all';

        // Trigger map filtering
        const filterBtn = document.querySelector(`.filter-btn[data-day="${targetDay}"]`);
    ...
    if (scheduleTitle) {
        ...
        scheduleTitle.title = 'Click to sync map & view today\'s schedule';
        ...
        scheduleTitle.addEventListener('click', () => {
            const currentDayId = syncToToday(); // Sync map to today
            ...
            const isDay = (id) => currentDayId === id ? 'border: 2px solid #00f0ff; transform: scale(1.05);' : 'opacity: 0.6;';
            ...
            modal.innerHTML = `
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(160px, 1fr));gap:15px;">
                    <div style="background:#1a1a1a;padding:15px;border-radius:12px;border-top:4px solid #00f0ff; ${isDay('1')}">
                        <h3 style="color:#00f0ff;margin-top:0;">MONDAY ${currentDayId === '1' ? '★' : ''}</h3>
                    ...
                    <div style="background:#1a1a1a;padding:15px;border-radius:12px;border-top:4px solid #2a2a2a; ${isDay('2')}">
                        <h3 style="color:#666;margin-top:0;">TUESDAY ${currentDayId === '2' ? '★' : ''}</h3>
                    ...
                    <div style="background:#1a1a1a;padding:15px;border-radius:12px;border-top:4px solid #8b4513; ${isDay('3')}">
                        <h3 style="color:#8b4513;margin-top:0;">WEDNESDAY ${currentDayId === '3' ? '★' : ''}</h3>
                    ...
                    <div style="background:#1a1a1a;padding:15px;border-radius:12px;border-top:4px solid #ff9d00; ${isDay('4')}">
                        <h3 style="color:#ff9d00;margin-top:0;">THURSDAY ${currentDayId === '4' ? '★' : ''}</h3>
                        <p style="font-size:13px;margin:5px 0;"><strong>07:30</strong> Briefing</p>
                        <p style="font-size:13px;margin:5px 0;"><strong>08:30</strong> Day 4 Scan (Orange)</p>
                    </div>
                    <div style="background:#1a1a1a;padding:15px;border-radius:12px;border-top:4px solid #ffffff; ${isDay('5')}">
                        <h3 style="color:#fff;margin-top:0;">FRIDAY ${currentDayId === '5' ? '★' : ''}</h3>
    ...
    if (saveBtn) {
        ...
        deployBtn.className = 'planner-btn'; deployBtn.id = 'prepare-deploy-btn';
        deployBtn.style.marginLeft = '10px'; deployBtn.style.borderColor = '#ff9d00'; deployBtn.style.color = '#ff9d00';
        ...
        deployBtn.addEventListener('click', () => {
            const data = polygons.map(p => ({ lat: p.lat, lng: p.lng, angle: p.angle, color: p.color, name: p.name, type: p.type, groupId: p.groupId, day: p.day }));
            ...
            modal.innerHTML = `<h3>READY FOR DEPLOYMENT</h3><textarea id="deploy-code" style="width:100%;height:150px;">${code}</textarea><div><button id="close-deploy">CLOSE</button><button id="copy-deploy">COPY</button></div>`;
            document.body.appendChild(modal);
            document.getElementById('copy-deploy').onclick = () => { document.getElementById('deploy-code').select(); document.execCommand('copy'); };
            document.getElementById('close-deploy').onclick = () => modal.remove();
        });
    }

    const coordDisplay = L.control({ position: 'bottomleft' });
    coordDisplay.onAdd = function () { this._div = L.DomUtil.create('div', 'coord-box'); return this._div; };
    ...
    function createObject(lat, lng, angle = 0, color = '#00f0ff', name = '', type = 'sector', groupId = null, dayOverride = null) {
        ...
        const marker = L.marker([lat, lng], { draggable: true, icon: L.divIcon({ className: 'drag-handle-container', html: '<div></div>', iconSize: [12, 12] }) });
        
        let day = dayOverride;
        if (!day) {
            const n = (name || '').toLowerCase();
            if (color === '#ff3e3e' || n.includes('block')) day = 'roadblock';
        ...
        layer.on('click', (e) => { L.DomEvent.stopPropagation(e); selectPolygon(polygons.indexOf(obj)); });
        marker.on('drag', (e) => { const newPos = e.target.getLatLng(); obj.lat = newPos.lat; obj.lng = newPos.lng; redrawObject(obj); });
        return obj;
    }

    function redrawObject(p) {
        const latPerMeter = 1 / 111320; const lonPerMeter = 1 / (40075000 * Math.cos(p.lat * Math.PI / 180) / 360); const rad = (p.angle * Math.PI) / 180;
        if (p.type === 'sector' || p.type === 'barrier') {
            const size = p.type === 'sector' ? {y:20, x:5} : {y:3, x:3};
            const corners = [{y: size.y, x: size.x}, {y: size.y, x: -size.x}, {y: -size.y, x: -size.x}, {y: -size.y, x: size.x}];
            const rotated = corners.map(c => [p.lat + (c.y * Math.cos(rad) - c.x * Math.sin(rad)) * latPerMeter, p.lng + (c.y * Math.sin(rad) + c.x * Math.cos(rad)) * lonPerMeter]);
            p.layer.setLatLngs(rotated);
        } else { p.layer.setLatLng([p.lat, p.lng]); }
    }
    ...
    function selectPolygon(index) {
        ...
        polygons.forEach((p, i) => {
            ...
            if (i === index) { p.marker.addTo(map); p.layer.setStyle({ color: '#fff', weight: 8, fillOpacity: 0.5 }); } else { map.removeLayer(p.marker); }
        });
        const editor = document.getElementById('editor-panel');
        if (index !== -1) {
            editor.style.display = 'flex'; const p = polygons[index];
    ...
    map.on('click', (e) => {
        if (!isAddMode) { selectPolygon(-1); return; }
        ...
        polygons.push(newObj); updateStats(); selectPolygon(polygons.length - 1);
    });

    document.getElementById('add-mode-btn').addEventListener('click', () => { isAddMode = !isAddMode; document.getElementById('add-mode-btn').classList.toggle('active', isAddMode); document.getElementById('map').style.cursor = isAddMode ? 'crosshair' : ''; });

    document.getElementById('rotation-slider').addEventListener('input', (e) => {
        if (selectedIndex === -1) return; const p = polygons[selectedIndex];
        p.angle = parseInt(e.target.value); document.getElementById('angle-value').innerText = `${p.angle}°`; redrawObject(p);
    });

    document.getElementById('poly-name-input').addEventListener('input', (e) => {
        if (selectedIndex === -1) return; const p = polygons[selectedIndex];
        p.name = e.target.value; p.layer.unbindTooltip(); if (p.name) p.layer.bindTooltip(p.name, { permanent: true, direction: 'center', className: 'poly-label' });
    });

    document.querySelectorAll('.color-swatch').forEach(swatch => {
        swatch.addEventListener('click', () => {
            document.querySelectorAll('.color-swatch').forEach(s => s.classList.remove('active')); swatch.classList.add('active');
            if (selectedIndex !== -1) { const p = polygons[selectedIndex]; p.color = swatch.getAttribute('data-color'); p.layer.setStyle({ color: p.color }); }
        });
    });

    document.getElementById('delete-poly-btn').addEventListener('click', () => {
        if (selectedIndex === -1) return; const p = polygons[selectedIndex];
        map.removeLayer(p.layer); map.removeLayer(p.marker); polygons.splice(selectedIndex, 1); updateStats(); selectPolygon(-1);
    });
    ...
    async function loadMission() {
        let sourceData = BAKED_DATA; polygons = [];
        try {
        ...
        updateStats(); selectPolygon(-1);
    }

    function updateStats() {
        const statsPill = document.getElementById('poly-stats');
        if (statsPill) { const sectors = polygons.filter(p => p.type === 'sector').length; const roadblocks = polygons.filter(p => p.day === 'roadblock').length; statsPill.innerHTML = `<strong>${sectors}</strong> SECTORS | <strong>${roadblocks}</strong> ROAD CLOSURES`; }
    }

    document.querySelectorAll('#day-filters .filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const day = btn.getAttribute('data-day');
            document.querySelectorAll('#day-filters .filter-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active');
            polygons.forEach(p => { const show = (day === 'all') || (String(p.day) === day); if (show) { p.layer.addTo(map); } else { map.removeLayer(p.layer); map.removeLayer(p.marker); } });
            selectPolygon(-1);
        });
    });
    ...
    async function saveMission() {
        ...
        try {
            const resp = await fetch('/api/mission', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
            if (resp.ok && sBtn) { sBtn.innerText = '✓ SAVED'; setTimeout(() => sBtn.innerHTML = '<span class="icon">💾</span> SAVE MISSION', 2000); }
