document.addEventListener('DOMContentLoaded', () => {
    ...
    function createObject(lat, lng, angle = 0, color = '#00f0ff', name = '', type = 'sector', groupId = null, manualDay = null) {
        ...
        // Map selection disabled for field viewer
    ...
    const SAVE_KEY = 'exodigo_field_ops_v41';
    ...
    async function loadMission() {
        ...
        const targetKP = polygons.find(p => p.name && p.name.includes('תשתיות מרכזיות'));

        console.log('Focus Logic - isMobile:', isMobile, 'Target Found:', !!targetKP);

        if (isMobile && targetKP) {
            // MOBILE: Specific vertical drop and ultra-zoom
            const pos = targetKP.layer.getBounds().getCenter();
            map.setView([pos.lat - 0.0090, pos.lng], 30); 
        } else if (polygons.length > 0) {
            // DESKTOP: Original fitBounds behavior for total visibility
            try {
                const layers = polygons.map(p => p.layer).filter(l => l);
                if (layers.length > 0) {
