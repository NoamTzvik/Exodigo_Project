document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDaySelection(daySelected) {
        console.log('Strict Isolation Mode (v59):', daySelected);

        const timeline = document.getElementById('protocol-timeline');
        const mainTitle = document.getElementById('schedule-title'); // "WEEKLY PROTOCOL"
        ...
        if (daySelected === 'all') {
            ...
            // Focus Mode: Hide EVERYTHING except current concierge card and body
            if (mainTitle) mainTitle.style.display = 'none'; // Clear the main title for focus
            if (summaryOutline) summaryOutline.style.display = 'block';
            if (summaryH4) summaryH4.style.display = 'none'; 
            
            // Hide all cards except chosen day
            summaryItems.forEach(item => {
                const iDay = item.getAttribute('data-day');
                item.style.display = (iDay === daySelected) ? 'flex' : 'none';
            });

            // Hide all detailed protocols except chosen day
    ...
    const SAVE_KEY = 'exodigo_field_ops_v60';
