document.addEventListener('DOMContentLoaded', () => {
    ...
    function createObject(lat, lng, angle = 0, color = '#00f0ff', name = '', type = 'sector', groupId = null, manualDay = null) {
        ...
        if (day === null) {
            ...
            if (color === '#ff3e3e' || normalizedName.includes('road closure') || normalizedName.includes('חסימה')) day = 'roadblock';
            else if (color === '#ffff00' || normalizedName.includes('key point')) day = 'prescan';
            else if (color === '#00f0ff') day = 1;
            else if (color === '#2a2a2a') day = normalizedName.includes('חסימת ציר ד.מע') ? ['2', '4'] : 2;
            else if (color === '#8b4513' || color === '#00d166') day = 3;
            else if (color === '#ff9d00') day = normalizedName.includes('חסימת ציר ד.מע') ? ['2', '4'] : 4;
    ...
    map.on('click', (e) => selectPolygon(-1));
    ...
    document.querySelectorAll('#day-filters .filter-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            refreshMapFilter(daySelected);
            
            // Sidebar Update
            const dayOutlines = document.querySelectorAll('#protocol-timeline .day-outline');
            dayOutlines.forEach(outline => {
                outline.style.display = (outline.getAttribute('data-day') === daySelected) ? 'block' : 'none';
            });
        });
    });
    ...
    async function loadMission() {
        ...
        } catch (e) { console.log("Server fallback to baked data."); }
        ...
        // V34 FOCUS LOGIC
        const targetKP = polygons.find(p => p.name && (p.name.includes('תשתיות מרכזיות') || p.name.includes('מרכזיוות')));
        if (targetKP) {
            const pos = targetKP.layer.getBounds().getCenter();
            map.setView([pos.lat - 0.0090, pos.lng], 30);
