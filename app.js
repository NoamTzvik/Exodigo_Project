document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDaySelection(daySelected) {
        console.log('Aggressive Selection:', daySelected);
        ...
        if (summaryOutline) summaryOutline.style.display = 'none';
        summaryItems.forEach(item => item.style.display = 'none');
        dayOutlines.forEach(outline => outline.style.display = 'none');

        if (daySelected === 'all') {
            // ALL Mode: Show full summary, hide details
            ...
            // Focus Mode: Show only selected summary item + its detailed outline
            if (summaryOutline) summaryOutline.style.display = 'block';
            if (summaryH4) summaryH4.style.display = 'none'; // Clear the "Weekly Overview" title
            
            summaryItems.forEach(item => {
                const iDay = item.getAttribute('data-day');
                item.style.display = (iDay === daySelected) ? 'flex' : 'none';
            });

            dayOutlines.forEach(outline => {
                const oDay = outline.getAttribute('data-day');
                const isSelected = (oDay === daySelected);
    ...
    document.querySelectorAll('#day-filters .filter-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            updateDaySelection(btn.getAttribute('data-day'));
        });
    });

    // Handle internal Sidebar clicks (from Weekly Overview)
    ...
    const SAVE_KEY = 'exodigo_field_ops_v58';
