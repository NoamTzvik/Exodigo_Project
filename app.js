document.addEventListener('DOMContentLoaded', () => {
    ...
    function redrawObject(p) {
        ...
        if (p.type === 'sector' || p.type === 'barrier') {
            const size = p.type === 'sector' ? {y:20, x:5.5} : {y:3, x:3};
            const corners = [{y:size.y, x:size.x}, {y:size.y, x:-size.x}, {y:-size.y, x:-size.x}, {y:-size.y, x:size.y}];
    ...
    function selectPolygon(index) {
        ...
        polygons.forEach((p, i) => {
            ...
            if (i === index) { p.marker.addTo(map); p.layer.setStyle({ color: '#fff' }); }
    ...
    document.querySelectorAll('#day-filters .filter-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            polygons.forEach(p => {
                ...
                if (show) { if (!map.hasLayer(p.layer)) p.layer.addTo(map); if (!map.hasLayer(p.marker)) p.marker.addTo(map); }
                else { if (map.hasLayer(p.layer)) map.removeLayer(p.layer); if (map.hasLayer(p.marker)) map.removeLayer(p.marker); }
            });
        });
    });
    ...
    topoBtn.addEventListener('click', () => {
        ...
        document.getElementById('topo-legend').style.display = active ? 'block' : 'none';
